...
| `RRAI Constitution.md` (root) | Root-level constitution placeholder | `architecture/00 RRAI Constitution.md` or remain at root pending content | Decide once populated | Currently a placeholder; final home depends on whether it supersedes or complements `architecture/00 Architecture Migration Plan.md` | Content of `00_RRAI_Master_Context_v2_0.md` when introduced | Medium — ambiguous authority ordering until resolved | Next documentation-governance phase |
...
- The main open question is the eventual relationship between the root `RRAI Constitution.md` and the two documents named in the governance instructions (`00_RRAI_Master_Context_v2_0.md`, `01_RRAI_Platform_Architecture_v0_9.md`), neither of which currently exists in the repository. This should be resolved before further governed documents are added.
